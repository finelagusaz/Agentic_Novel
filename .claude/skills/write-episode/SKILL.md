...
3. **判定**:
   - 全員「問題なし」系 → ディスカッション終了、progress.md 更新、次ステップへ
   - 意見あり → エージェント間の直接メッセージを許容
...
**読者**は Step 5 で遅延スポーンする（team_name **なし**のサブエージェント）。
...
読者は **team_name なし**のサブエージェントとしてスポーンする（チームメンバーではない）。3名を並列で Task スポーンする。
...
5. REVISION_NEEDED の場合:
   ...
   - progress.md を更新:
     ...
     - step3, step4, step4d, step5, step6 のチェックを `[ ]` にリセット
...
6. FORCE_PASS の場合:
   - Step 6.5D は**スキップ**し、直接 Step 7 へ進む
