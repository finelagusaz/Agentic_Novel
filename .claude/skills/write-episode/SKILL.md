...
3. **ファイルが存在する場合**: Read で読み込み、YAML ブロックから `episode` と `current_step` を確認:
   a. **episode が引数と一致する場合 → 再開モード**
      ...
      - Step 1（チーム作成）は**必ず実行**（チームはセッション間で維持されない）
      ...
      - **再開時は全エージェントを未スポーン扱い**とする（revision_count > 0 でも初回は Task スポーン、SendMessage は使わない）
...
| Step 6 | `manager-review.md`, `reader-feedback-young-male.md`, `reader-feedback-adult-female.md`, `reader-feedback-veteran.md` |
| Step 6.5 | Step 6 と同じ + `current-direction.md`, `current-draft.txt` |
...
2. `workspace/revision-log.md` を作成（空テンプレート）:
   ...
   # リビジョン履歴 — 第{番号}話
   ```
3. **`workspace/progress.md` を作成**（後述のフォーマットに従い初期テンプレートを出力）
4. `mkdir -p archive/episode-{番号:2桁}` でアーカイブディレクトリを事前作成する
5. 前話のエピソード（episodes/ 内）が存在するか確認
6. revision_count = 0 として初期化
7. `story/handover-notes.md` を読み込み、`[DEFERRED:ep{今回の番号}]` のタグが付いた項目を `[ACTIVE]` に昇格する（該当項目がなければスキップ）
8. **progress.md を更新**: `current_step: "step0"`, `step_status: "completed"`, step0 を `[x]` に
...
**リビジョンの場合（revision_count > 0）** — ※ 再開モードの場合、revision_count > 0 でも初回のエージェント操作は SendMessage ではなく Task スポーンを使う（セッション間でエージェントは維持されないため）。

SendMessage で **author** に以下を指示:
...
`[x]` のリーダーは出力ファイルを Glob + Read で検証する。
検証成功なら再スポーンしない。未完了 `[ ]` のリーダーのみスポーンする。
...
各リーダーの検証成功時に progress.md の「Step 5 Detail」で該当リーダーを `[x]` に更新する。
...
4. PASS または FORCE_PASS の場合: progress.md を更新: step6 を `[x]`、`current_step: "step6"`, `step_status: "completed"`

5. REVISION_NEEDED の場合:
   - revision_count を +1
   - progress.md を更新:
     - `revision_count` を +1 した値に更新
     - `current_step: "step6.5"`, `step_status: "in_progress"`
     - step3, step4, step5, step6 のチェックを `[ ]` にリセット
...
editor からの完了報告を待ち、`workspace/consolidated-feedback.md` について**共通手順: エージェント出力の検証**に従う。

progress.md を更新: `current_step: "step6.5"`, `step_status: "completed"`
...
5. workspace/ の全ファイルを `archive/episode-{番号:2桁}/` にコピー（progress.md を含む）
