...
4. **方針ディスカッション**: 作者・担当者が方針に意見があれば議論（なければ即進行）
5. **作者（執筆/改稿）**: current-draft.txt を出力
6. **担当者（レビュー）**: manager-review.md を出力
7. **ドラフトディスカッション**: 作者・編集がレビュー所見に対し意見交換（なければ即進行）
8. **読者×3（並列フィードバック）**: reader-feedback-*.md を出力（サブエージェント、team_name なし）
9. **判定**: OK かつ 読者平均★≥3.5 → 確定 / 要修正 → Step 6.5D へ（最大3回）
   - **6.5D リビジョンディスカッション**: 三者で改稿方針を議論 → consolidated-feedback.md を出力 → Step 5 に戻る
10. **確定・保存**: episodes/ に保存、episode-summaries.md に追記、archive/ にアーカイブ
11. **チームシャットダウン**: 全メンバーにshutdown_request → TeamDelete
